...
<script type="text/javascript">
	function search() {
		...
		$.ajax({
			url : "/cosmos/framework/rolemanager/ServiceListWithNotMapped.action",
			data : {
				roleId : roleId,
				svcName : svcName,
				useYn : decode(useYn, '%'),
	...
	function setServiceTypes() {
		$.ajax({
			url : "/cosmos/framework/servicemanager/ServiceTypes.action",
...
<script type="text/javascript">
	...
	function save() {
		...
		if (cnt == 0) {
			...
			$.ajax({
				url : "/cosmos/framework/rolemanager/RoleServiceMappingApply.action",
...
function searchMappedServices() {
    ...
    $.ajax({
        url : "/cosmos/framework/rolemanager/ServiceListWithMapped.action",
...
<script type="text/javascript">
    ...
    function mappingRemove() {
        ...
        if (cnt == 0) {
            ...
            $.ajax({
                url : "/cosmos/framework/rolemanager/RoleServiceMappingRemove.action",
                data : "jsonParam=" + jsonParam,
                success : callbackSave
            });
        }
