...
<script type="text/javascript">
    function setMenu(serviceUrl) {
        ...
        $.ajax({
            url : "/cosmos/common/auth/SelectMenu.action",
            data : {
						...
						<div id="initDisplay">
						    <%
						       if(selectedServiceId == null || selectedServiceId.equals("")){
						    	   out.println("<a href=\"/cosmos/common/common/Overview.action\">System Overview</a>&nbsp;&nbsp;<font color=\"#808080\"> : Cosmos webapplication framework</font>");
						       }
						    %>
						</div>
						    ...
						    <%
				                 ...
				                 while(roleServiceIter.hasNext()){
				                     ...
				                     for(ServiceDto svcDto:svcList){
				                    	 out.print("<a id=\"service_" + svcDto.getSvcId() + "\" onClick=\"subClick('" + svcDto.getSvcId() + "', '/cosmos" + svcDto.getSvcPrefix() + svcDto.getSvcUrl() + "/Main.action')\" style=\"cursor:hand;\">" + svcDto.getSvcName() + "</a>");
				                    	 out.print(" | ");
				                     }
				                     out.println("</div>");
				                 }
