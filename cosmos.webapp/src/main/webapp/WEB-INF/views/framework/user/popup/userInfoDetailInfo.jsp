...
<script type="text/javascript">
    ...
    function modifyUser() {
    	document.location.href = "/cosmos/framework/usermanager/UserModifyForm.action?userId=" + '<c:out value="${userDto.userId}"/>';
    }
    ...
    function deleteUser(){
        ...
        $.ajax({
            url : "/cosmos/framework/usermanager/UserDelete.action",
            data : "userId=" + userId,
    ...
    function deleteUserPhoto(){
        ...
        if( pictureFile == '' || pictureFile == null){
	        ...
	        $.ajax({
	            url : "/cosmos/common/filecontrol/FileDelete.action",
	            data : {
	                fileId : pictureFile,
	                infoType : infoType },  
	            success : callback,
    ...
    function useYn(value){
        ...
        $.ajax({
            url : "/cosmos/framework/usermanager/ModifyUseYn.action",
            data : {
            	userId : userId,
                useYn : value
                },            
    ...
    function setUserRole() {
        document.location.href = "/cosmos/framework/usermanager/UserRole.action?userId=" + '<c:out value="${userDto.userId}"/>';
    }    
                    ...
                    <div id="fileField"  style="display: block;">
                        <c:choose>
                            <c:when test="${!empty userPhoto}">
                                <img src="/cosmos/image?file=${userPhoto}" width="90px" height="90px"/>
                            </c:when>
                            <c:otherwise>
                                <img src="/cosmos/resources/images/common/photo_samplebg.gif" width="90px" height="95px"/>
                            </c:otherwise>
                        </c:choose>
                    </div>
                </td>
