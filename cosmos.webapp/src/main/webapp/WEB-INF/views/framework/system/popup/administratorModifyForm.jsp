...
<script type="text/javascript">
    ...
    function callbackSearch(data) {
        ...
        var script_template = '<tr>'
            ...
            + '    <td> {mobilePhone} </td> '
            + '    <td> {useYn} </td>'
            + '    <td> <button class="word6" onclick="select(\'{userId}\')">select</button></td>'
            + '</tr>';
...
<script type="text/javascript">
	function select(userId) {
	    $.ajax({
	        url : "/cosmos/framework/sysproperty/AdministratorModify.action",
	        data : "userId=" + userId,
	        success : callbackSelect
	    });
	
	}
	function callbackSelect(data) {
		pageLog(data.message);
