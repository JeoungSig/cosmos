...
<script type="text/javascript">
    function modify() {
    	document.location.href = "/photome/business/resmanage/booth/BoothInfoUpdateForm.action?boothId=" + <c:out value="${boothDto.boothId}" />;
    }
</script>
		...
		<table id="detail_table" class="detail_table" cellpadding="0" cellspacing="0" style="width: 750px;" >
			...
			<tr>
               ...
               <tr>
                   ...
                   <td class="value" nowrap><c:out value="${boothDto.technicianName}" /></td>
               </tr>
			...
			<tr>
				...
				<td width="*" align="left">
					<c:if test="${sessionUser.userInfo.userId == 'admin' }" >
					    <button class="word4" onclick="deleteBoothInfo();">delete</button>
					</c:if>
					<c:choose>
					   <c:when test="${boothDto.useYn == 'Y' }" ><button class="word4" onclick="setBoothUseYN('N');">disable</button></c:when>
					   <c:when test="${boothDto.useYn == 'N' }" ><button class="word4" onclick="setBoothUseYN('Y');">activate</button></c:when>
					</c:choose>
