...
<script type="text/javascript">
    function depositCancel() {
        ...
        $.ajax({
            url : "/photome/business/money/manage/deposit/BankDepositCancel.action",
    ...
    function changeStatus(status) {
    	...
    	var depositId = '<c:out value="${moneyDepositDto.depositId}"/>';
        $.ajax({
            url : "/photome/business/money/manage/deposit/BankDepositChangeStatus.action",
            data : {
            	depositId : depositId,
            	status : status
            },
            success : callbackChangeStatus
        });
    }
    
    function callbackChangeStatus(data) {
        var returnValue = {
        ...
        <table id="detail_table" class="detail_table" cellpadding="0" cellspacing="0">
            ...
            <tr>
                ...
                <td class="value" colspan="4" align="right">
                    <c:choose>
                        <c:when test="${moneyDepositDto.status == 'COLLECT_FINISH' }">
                            <button class="word7" onclick="changeStatus('COLLECTING')">Take Back</button>
                            <button class="word7" onclick="changeStatus('FINALCHK_FINISH')">Finalize</button>
                        </c:when>
                    </c:choose>
                    <c:if test="${ sessionUser.userId == 'admin' }">
                         <button class="word7" onclick="changeStatus('COLLECT_FINISH')">Take Back</button>
