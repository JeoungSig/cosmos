...
public class MySqlMoneyCollectRptMapDao extends CosmosBaseDao implements MoneyCollectRptMapDao {
	...
	public Map<String, String> getCollectRealIncomeMap(HashMap<String, String> param) throws Exception {
		...
		for (MoneyCollectDto dto : list) {
			realIncomeMap.put(dto.getBoothId() + "|" + dto.getCollectMonth(), roundOff(Double.parseDouble(dto.getCollectRealIncome())));
		}
	...
	public Map<String, String> getCollectRealIncomeMapByGroup(HashMap<String, String> param) throws Exception {
		...
		for (MoneyCollectDto dto : list) {
			realIncomeMap.put(dto.getCollectId(), roundOff(Double.parseDouble(dto.getCollectRealIncome())));
		}
	...
	public Map<String, String> getRefundMap(HashMap<String, String> param) throws Exception {
		...
		for (RefundDto dto : list) {
			refundMap.put(dto.getBoothId() + "|" + dto.getRefundDt(), roundOff(Double.parseDouble(dto.getRefundAmount())));
		}
	...
	public Map<String, String> getRefundMapByGroup(HashMap<String, String> param) throws Exception {
		...
		// String temp = "";
		for (RefundDto dto : list) {
			refundMap.put(dto.getRefundId(), roundOff(Double.parseDouble(dto.getRefundAmount())));
	...
	public Map<String, String> getBoothRentFeeAmountMap(String boothId, String rentFeeType, String year) throws Exception {
		...
		if (StringUtils.equals(rentFeeType, "FIXED_MONEY")) {
				...
				if (paramYear <= curYear) {
					if (filter) {
						...
						if (dtoMonth <= curMonth) {
							rentFeeMap.put(dto.getBoothId() + "|" + dto.getRentMonth(), roundOff(Double.parseDouble(dto.getRentAmount())));
						}
					} else {
						// 조회 년도가 현재 년도보다 작은경우
						rentFeeMap.put(dto.getBoothId() + "|" + dto.getRentMonth(), roundOff(Double.parseDouble(dto.getRentAmount())));
			...
			for (MoneyCollectDto dto : collectList) {
				collectMap.put(dto.getBoothId() + "|" + dto.getCollectMonth(), roundOff(Double.parseDouble(dto.getCollectRealIncome())));
			}
	...
	@Override
	public Map<String, String> getCollectAmtPerPerson(String collectYear, String userId) throws Exception {
		HashMap<String, String> param = new HashMap<String, String>();
		param.put("collectYear", collectYear);
		param.put("userId", userId);

		List<MoneyCollectDto> collectList = getSqlSession().selectList("mySqlMoneyCollectRptMapDao.getCollectAmtPerPerson", param);

		Map<String, String> collectAmtMap = new HashMap<String, String>();
		for (MoneyCollectDto dto : collectList) {
			collectAmtMap.put(dto.getCollectCollector() + "|" + dto.getCollectMonth(), dto.getCollectRealIncome());
		}

		return collectAmtMap;
	...
	public List<UserDto> getCollectorList(String collectYear, String userId) throws Exception {
		...
		param.put("collectYear", collectYear);
		param.put("userId", userId);
		List<UserDto> collectorList = getSqlSession().selectList("mySqlMoneyCollectRptMapDao.getCollectorList", param);
