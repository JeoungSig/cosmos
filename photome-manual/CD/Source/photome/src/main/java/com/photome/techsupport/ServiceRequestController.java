...
public class ServiceRequestController {
	...
	public ModelAndView main(HttpServletRequest request) throws Exception {
		...
		/*
		 * if (StringUtils.equals(sessionUser.getUserInfo().getUserId(), "admin")) { List<UserDto> technicianList = techSupport.getTechnicianList(new
		 * HashMap<String, String>()); StringBuffer technicians = new StringBuffer("<option selected></option>"); for (UserDto dto : technicianList) {
		 * technicians.append("<option value=\"" + dto.getUserId() + "\" >" + dto.getFirstName() + "</option>"); }
		 * mav.addObject("technicianListOptions", technicians.toString()); } else { mav.addObject("technicianListOptions", "<option value=\"" +
		 * sessionUser.getUserInfo().getUserId() + "\" selected>" + sessionUser.getUserInfo().getFirstName() + "</option>"); }
		 */

		List<UserDto> writerList = techSupport.getServiceRequesterList(new HashMap<String, String>());
		StringBuffer writers = new StringBuffer("<option selected></option>");
		for (UserDto dto : writerList) {
			writers.append("<option value=\"" + dto.getUserId() + "\"" + (StringUtils.equals(sessionUser.getUserInfo().getUserId(), dto.getUserId()) ? " selected" : "") + " >" + dto.getFirstName()
					+ "</option>");
		}
		mav.addObject("writerOptions", writers.toString());
	...
	public Map<String, Object> getHistoryList(HttpServletRequest request) throws Exception {
		...
		if(StringUtils.equalsIgnoreCase(groupId, "ALL")) groupId = null;
		...
		/*
		 * CosmosSessionUserInfo sessionUser = auth.getSessionUserInfo(request); if (StringUtils.equals(sessionUser.getUserInfo().getUserId(),
		 * "admin")) { if (StringUtils.isNotBlank(creator)) param.put("creator", creator); } else { param.put("creator",
		 * sessionUser.getUserInfo().getUserId()); }
		 */
	...
	public ModelAndView registHistoryForm(HttpServletRequest request) throws Exception {
		...
		CosmosSessionUserInfo sessionUser = auth.getSessionUserInfo(request);
		/*
		 * if (StringUtils.endsWith(sessionUser.getUserInfo().getUserId(), "admin")) { mav.addObject("groupListOptions",
		 * resManage.getGroupComboOptions("")); } else { mav.addObject("groupListOptions",
		 * resManage.getGroupComboOptions(sessionUser.getUserInfo().getUserId(), "")); }
		 */
		mav.addObject("groupListOptions", resManage.getGroupComboOptions(""));
	...
	public ModelAndView modifyHistoryForm(HttpServletRequest request) throws Exception {
		...
		/*
		 * if (StringUtils.endsWith(sessionUser.getUserInfo().getUserId(), "admin")) { mav.addObject("groupListOptions",
		 * resManage.getGroupComboOptions(historyDto.getGroupId())); } else { mav.addObject("groupListOptions",
		 * resManage.getGroupComboOptions(sessionUser.getUserInfo().getUserId(), historyDto.getGroupId())); }
		 */
	...
	@RequestMapping(value = "/ServiceRequestDelete", produces = "application/json")
	@ResponseBody
	public Map<String, Object> deleteHistory(HttpServletRequest request) throws Exception {

		Map<String, Object> model = new HashMap<String, Object>();

		String historyId = request.getParameter("historyId");
		int updateCnt = techSupport.deleteHistory(historyId);
