...
import com.cosmos.framework.excel.ExcelData;
...
public class MoneyManagerController {
	...
	@RequestMapping("/MoneyCollectListForExcel")
	public ModelAndView MoneyCollectListForExcel(HttpServletRequest request) throws Exception {
		...
		return new ModelAndView("excelView", CosmosConstants.EXCEL_DATA, this.genExcelListForMoneyCollectList(list));
	}

	private ExcelData genExcelListForMoneyCollectList(List<MoneyCollectDto> list) throws Exception {
		ExcelData excelData = new ExcelData();
		excelData.setSheetName("Status of Money Collect");
		String header[] = { "ID", "Group", "Booth", "Status", "Collect", "Bank Amt.", "Coin mtr", "Coin mtr(bfr)", "Collector", "Onsite pay", "Collect Date", "Banking date", "Write date" };
		excelData.setHeader(header);

		List<Map<String, String>> excelList = new ArrayList<Map<String, String>>();
		Map<String, String> aRow = null;
		for (MoneyCollectDto collectDto : list) {
			aRow = new HashMap<String, String>();
			aRow.put("ID", collectDto.getCollectId());
			aRow.put("Group", collectDto.getGroupName());
			aRow.put("Booth", collectDto.getBoothName());
			aRow.put("Status", collectDto.getCollectStatusName());
			aRow.put("Collect", collectDto.getCollectRealIncome());
			aRow.put("Bank Amt.", collectDto.getBankAmount());
			aRow.put("Coin mtr", collectDto.getCoinMtrNow());
			aRow.put("Coin mtr(bfr)", collectDto.getCoinMtrBfr());
			aRow.put("Collector", collectDto.getCollectCollectorName());
			aRow.put("Onsite pay", collectDto.getOnsiteIsPaid());
			aRow.put("Collect Date", collectDto.getCollectDate());
			aRow.put("Banking date", collectDto.getBankDt());
			aRow.put("Write date", collectDto.getCreateDt());
			excelList.add(aRow);
		}
		excelData.setData(excelList);
