...
public class MoneyCollectService implements MoneyCollect {
	...
	public boolean checkBeforCoinMeter() throws Exception {
		...
		param.put("systemCd", "PHOTOME");
		param.put("categoryCd", "CHK_COIN_MTR");
		param.put("codeValue", "CHK_BEFORE_COIN_MRT");

		boolean retrunValue = true;

		CodeDto code = codeDao.getCodeInfo(param);
		if (code != null) {
			String codeValue = code.getUseYn();
			if (StringUtils.equalsIgnoreCase(codeValue, "N")) {
				retrunValue = false;
			}
		}
		return retrunValue;
